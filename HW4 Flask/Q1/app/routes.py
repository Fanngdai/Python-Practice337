...
from app import app
from app.forms import SubmissionForm, EditForm, LoginForm, RegistrationForm
...
def editpage(postid):
    if current_user.is_authenticated:
        current_post = Post.query.filter_by(id=postid)[0]
        form = EditForm(text=current_post.body)
        if form.validate_on_submit():
            flash('Topic {} has been Modified.'.format(current_post.title))
            current_post.body = form.text.data;
            db.session.commit()

            try:
                fout = open("Modified/"+postid+".txt", "w")
                fout.write(form.text.data)
                # fout.write("Title: " + current_post.title + " \n\n" + form.text.data + "\n\nBy: " + str(current_user.username) + " at " + str(current_post.timestamp))
                fout.close()
            except IOError:
                print("Unable to write to file.")

            return redirect(url_for('index'))
        return render_template('edit.html',  title='modify Entry', form=form, post=current_post)
    return render_template('notloggedin.html')


# An extra piece of code used to delete posts
